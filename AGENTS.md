...
(context, layout, commands, gate, invariants) lives in `docs/project.md`, imported at the end; if it
...
- **Consult the declared documentation source, not recall**, for any library, CLI or cloud service and
  any version-dependent value (flags, ports, roles, defaults, formats); `docs/project.md` names the
...
declared in `docs/project.md` and receives work only through integration. Ask which option and wait:
...
1. **Discuss.** Open `docs/backlog.md` (records what is already arbitrated; receives what this phase
   ...
   structured-elicitation affordance named in `docs/project.md`, if any.
...
5. **Verify.** Run the full project gate as declared in `docs/project.md` (run, not described), then a
...
6. **Wrap.** (a) Update the backlog in the branch. (b) Write the handoff in
   ...
   commit); documentation-only changes on paths declared in `docs/project.md` may merge locally,
   **except `docs/project.md` itself**, which goes through a PR like code. (d) Tag if the spec called
...
7. **Improve.** **Never skipped, even when the work went fine.** The question: what should the gate
   ...
   cheapest durable form: **`docs/project.md`** for a judgement call, **a test** for a structural
...
who knows the criteria writes to them. _Observable: no read of `docs/reviews/` in the trace of a
...
- **The gate perimeter is decided by location**, declared once in `docs/project.md`: inside is 100%,
...
- **Generated artefacts are declared, not assumed.** Code no human wrote and no test can reach is not
  source, so it was never inside; `docs/project.md` names each generator and its exclusion, and a
...
Two regimes; `docs/project.md` declares which document belongs to which.
...
- **A convention that must persist goes in `docs/project.md`**, not session memory: memory is invisible
