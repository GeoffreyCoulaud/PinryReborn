...
- **Outside (not measured)**:
  ...
  - `...persistence.sqlite.models.query.Q*` and every class annotated
    ...
    no test can reach and nobody wrote. This is the only override of the generic gate-perimeter
    rule. One other rule of `AGENTS.md` is narrowed here, and it is the merge convention under
    Conventions.
