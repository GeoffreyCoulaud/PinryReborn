...
- **Two unrelated `PersistenceException` types.** `UserModelDoesNotExistError` (`api-persistence-sqlite/.../exceptions/UserModelDoesNotExistError.kt`) extends the project's own `PersistenceException` (`...sqlite.exceptions.PersistenceException`, a `java.lang.Exception` subtype), which is a DIFFERENT type from the `jakarta.persistence.PersistenceException` the password repository's `catch` is on. The two are siblings on `Throwable`, so `UserModelDoesNotExistError` would not be caught either way; the user lookup (`ActiveUserModels.resolve`) is kept OUTSIDE the try to keep the "user absent" and "insert constraint" error paths distinct.
