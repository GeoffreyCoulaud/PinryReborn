 ...
 * driver reads into its `SQLiteConfig`. Extracted as a pure function so the fallback branch is
 * unit-testable without touching the process environment or building a real database.
 *
 * Deliberately does NOT set `transaction_mode=IMMEDIATE`: the datasource below is constrained
 * to a single connection (option A), so there is no pool contention and thus no need for
