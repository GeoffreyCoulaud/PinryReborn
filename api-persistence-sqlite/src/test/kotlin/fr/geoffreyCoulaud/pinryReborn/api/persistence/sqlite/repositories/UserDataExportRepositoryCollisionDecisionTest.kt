...
/**
 * Focused unit tests for the collision decision in [UserDataExportRepository].
 *
 ...
 * [fr.geoffreyCoulaud.pinryReborn.api.persistence.sqlite.UserDataExportRepositoryTest] pins the
 ...
 */
class UserDataExportRepositoryCollisionDecisionTest {
    @Test
    fun `Given a unique-constraint violation, Then isUniqueConstraint returns true`() {
        ...
        // When
        val result = UserDataExportRepository.isUniqueConstraint(error)
        // Then
        assertTrue(result)
    }
    ...
    @Test
    fun `Given a unique-constraint violation, Then translateIfCollision throws ExportAlreadyInProgressException`() {
        // Given
        val cause =
            SQLiteException(
                "[SQLITE_CONSTRAINT_UNIQUE] A UNIQUE constraint failed",
        ...
        // When / Then
        val thrown =
            assertThrows(ExportAlreadyInProgressException::class.java) {
                UserDataExportRepository.translateIfCollision(error)
            }
