...
class PinryRuleSetProviderTest {
    /**
     * Two of the three rules earn an activation red against the real sources, which is what proves
     * they are loaded. `QueryBeanConstructedByQualifiedName` cannot: no production source constructs
     * a query bean by qualified name, so it reports nothing whether it is registered or absent from
     * the rule set entirely, and detekt fails the build for neither. Its registration is asserted
     * here or nowhere.
     */
    @Test
    fun `Given the provider, Then its rule set carries exactly the three expected rules`() {
        // Given
