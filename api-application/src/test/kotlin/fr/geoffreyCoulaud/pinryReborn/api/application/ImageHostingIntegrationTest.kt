...
class ImageHostingIntegrationTest : IntegrationTest() {
    ...
    private fun createUserAndPin(username: String, password: String): Pair<UUID, UUID> {
    ...
    fun `Given a pin owned by someone else, Then uploading an image returns 403`() {
        // Given
        val (_, pinId) = createUserAndPin("imgowner2", "password123")
        userCreator.createUserWithPassword("imgintruder", "password456")

        // When / Then
        given()
            .auth().preemptive().basic("imgintruder", "password456")
            .multiPart("file", fixture("sample.png"), "image/png")
            .`when`().put("/api/v1/pins/$pinId/image")
            .then()
            .statusCode(403)
